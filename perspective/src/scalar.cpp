...
tscalar_to_python(const t_tscalar& s)
{
    ...
    switch (s.m_type)
    {
        case DTYPE_INT64:
        {
            return PyLong_FromLongLong(s.get<t_int64>());
        }
        break;
        case DTYPE_INT32:
        {
            return PyInt_FromLong(s.get<t_int32>());
        }
        break;
        case DTYPE_INT16:
        {
            return PyInt_FromLong(s.get<t_int16>());
        }
        break;
        case DTYPE_INT8:
        {
            return PyInt_FromLong(s.get<t_int8>());
        }
        break;
        case DTYPE_UINT64:
        {
            return PyLong_FromLongLong(s.get<t_uint64>());
        }
        break;
        case DTYPE_UINT32:
        {
            return PyInt_FromLong(s.get<t_uint32>());
        }
        break;
        case DTYPE_UINT16:
        {
            return PyInt_FromLong(s.get<t_uint16>());
        }
        break;
        case DTYPE_UINT8:
        {
            return PyInt_FromLong(s.get<t_uint8>());
        }
        break;
        case DTYPE_FLOAT64:
        {
            if (!s.is_valid())
                return PyFloat_FromDouble(
                    std::numeric_limits<t_float64>::quiet_NaN());
            return PyFloat_FromDouble(s.get<t_float64>());
        }
        break;
        case DTYPE_FLOAT32:
        {
            if (!s.is_valid())
                return PyFloat_FromDouble(
                    std::numeric_limits<t_float64>::quiet_NaN());
            return PyFloat_FromDouble(s.get<t_float32>());
        }
        break;
        case DTYPE_NONE:
        {
        ...
        case DTYPE_TIME:
        {
            t_time t = s.get<t_time>();
            struct tm tstruct;
            bool rcode = t.as_tm(tstruct);
            if (rcode)
            {
                if (t.year(tstruct) <= 1900)
                {
            ...
            else
            {
                return PyString_FromString(
                    "Could not return datetime value.");
            }
        }
        break;
        case DTYPE_STR:
        {
            return PyString_FromString(s.get<const char*>());
        }
        break;
        case DTYPE_BOOL:
        {
...
t_int64
py_to_int(PyObject* pyo, t_bool& valid)
{
    if (PyInt_Check(pyo))
    {
        t_int64 value = PyInt_AsLong(pyo);
        if (value != -1 || (value == -1 && !PyErr_Occurred()))
        {
            valid = true;
            return value;
        }
    ...
    if (PyLong_Check(pyo))
    {
        t_int64 value = PyLong_AsLongLong(pyo);
        if (value != -1 || (value == -1 && !PyErr_Occurred()))
        {
            valid = true;
            return value;
...
t_float64
py_to_float(PyObject* pyo, t_bool& valid)
{
    t_float64 value = 0;
    if (PyFloat_Check(pyo))
    {
        value = PyFloat_AsDouble(pyo);
        valid = !(value == -1.0 && PyErr_Occurred());
    }
    else
    {
        valid = false;
...
t_tscalar
python_to_tscalar(t_dtype dtype, PyObject* pyo)
{
    if (!PyDateTimeAPI)
        PyDateTime_IMPORT;
    ...
    switch (dtype)
    {
        case DTYPE_INT64:
        {
            python_to_tscalar_helper_int<t_int64>(pyo, rval);
        }
        break;
        case DTYPE_INT32:
        {
            python_to_tscalar_helper_int<t_int32>(pyo, rval);
        }
        break;
        case DTYPE_INT16:
        {
            python_to_tscalar_helper_int<t_int16>(pyo, rval);
        }
        break;
        case DTYPE_INT8:
        {
            python_to_tscalar_helper_int<t_int8>(pyo, rval);
        }
        break;
        case DTYPE_UINT64:
        {
            python_to_tscalar_helper_int<t_uint64>(pyo, rval);
        }
        break;
        case DTYPE_UINT32:
        {
            python_to_tscalar_helper_int<t_uint32>(pyo, rval);
        }
        break;
        case DTYPE_UINT16:
        {
            python_to_tscalar_helper_int<t_uint16>(pyo, rval);
        }
        break;
        case DTYPE_UINT8:
        {
            python_to_tscalar_helper_int<t_uint8>(pyo, rval);
        }
        break;
        case DTYPE_FLOAT64:
        {
            python_to_tscalar_helper_float<t_float64>(pyo, rval);
        }
        break;
        case DTYPE_FLOAT32:
        {
            python_to_tscalar_helper_float<t_float32>(pyo, rval);
        }
        break;
        case DTYPE_NONE:
        {
        ...
        case DTYPE_DATE:
        {
            if (PyDate_CheckExact(pyo))
            {
                rval.set(t_date(PyDateTime_GET_YEAR(pyo),
                                PyDateTime_GET_MONTH(pyo),
                                PyDateTime_GET_DAY(pyo)));
            }
        }
        break;
        case DTYPE_TIME:
        {
            if (PyDateTime_CheckExact(pyo))
            {
                t_int64 ticks =
                    to_gmtime(PyDateTime_GET_YEAR(pyo),
                              PyDateTime_GET_MONTH(pyo),
                              PyDateTime_GET_DAY(pyo),
                              PyDateTime_DATE_GET_HOUR(pyo),
                              PyDateTime_DATE_GET_MINUTE(pyo),
                              PyDateTime_DATE_GET_SECOND(pyo));
                ticks *= 1000000;
                ticks += PyDateTime_DATE_GET_MICROSECOND(pyo);
                rval.set(t_time(ticks));
            }
        }
        break;
        case DTYPE_STR:
        {
            ...
            {
                ssize_t Size = PyString_GET_SIZE(pyo);
                if (Size >= 0)
                {
                    const char* itemValue =
                        get_interned_cstr(PyString_AS_STRING(pyo));
                    rval.set(itemValue);
                }
            }
            ...
            {
                PyObjectPtr s(
                    PyUnicode_AsEncodedString(pyo, "UTF-8", ""));
                const char* itemValue =
                    get_interned_cstr(PyString_AS_STRING(s.get()));
                rval.set(itemValue);
            }
            else
            {
                PyObjectPtr py_str(PyObject_Str(pyo));
                if (py_str.get())
                {
                    const char* itemValue = get_interned_cstr(
                        PyString_AS_STRING(py_str.get()));
                    rval.set(itemValue);
                }
            }
        }
...
t_tscalar
python_to_tscalar(PyObject* pyo)
{
    t_tscalar rval;
    rval.set(t_none());
    if (!PyDateTimeAPI)
    ...
    else if (PyBool_Check(pyo))
    {
        t_int64 value = (pyo == Py_True);
        rval.set(value);
        return rval;
    }
    else if (PyInt_Check(pyo))
    {
        t_int32 value = PyInt_AsLong(pyo);
    ...
    {
        t_float64 value = PyFloat_AsDouble(pyo);
    ...
    {
        PyObject* s = PyUnicode_AsEncodedString(pyo, "UTF-8", "");
        rval.set(PyString_AS_STRING(s));
        Py_DECREF(s);
        return rval;
    }
    else if (PyDateTime_CheckExact(pyo))
    {
        t_int64 ticks = to_gmtime(PyDateTime_GET_YEAR(pyo),
                                  PyDateTime_GET_MONTH(pyo),
                                  PyDateTime_GET_DAY(pyo),
                                  PyDateTime_DATE_GET_HOUR(pyo),
                                  PyDateTime_DATE_GET_MINUTE(pyo),

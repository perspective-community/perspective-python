...
PERSPECTIVE_EXPORT t_tscalar calc_delta(t_value_transition trans,
                                        t_tscalar oval,
                                        t_tscalar nval);

PERSPECTIVE_EXPORT t_tscalar calc_newer(t_value_transition trans,
                                        t_tscalar oval,
                                        t_tscalar nval);
...
class PERSPECTIVE_EXPORT t_gnode
{
  public:
	...
	t_gnode(const t_schema& tblschema,
			const t_schema& port_schema);
    t_gnode(t_gnode_processing_mode mode,
    ...
    void _process();
    void _process_self();
    void _register_context(const t_str& name,
                           t_ctx_type type,
                           t_int64 ptr);
  ...
  protected:
    ...
    void notify_context(const t_table& flattened,
                        const t_ctx_handle& ctxh);

    template <typename CTX_T>
    void notify_context(CTX_T* ctx,
    ...
    void _process_helper(const t_column* fcolumn,
  ...
  private:
    void
    populate_icols_in_flattened(const std::vector<t_rlookup>& lkup,
                                t_table_sptr& flat) const;
                        ...
                        const t_ctx_handle& ctxh)
{
    ...
    const t_table& prev =
        *(m_oports[PSP_PORT_PREV]->get_table().get());
    const t_table& current =
...
template <typename CTX_T>
void
t_gnode::notify_context(CTX_T* ctx,
                        const t_table& flattened,
                        const t_table& delta,
                        const t_table& prev,
                        const t_table& current,
...
template <typename CTX_T>
void
t_gnode::update_context_from_state(CTX_T* ctx,
                                   const t_table& flattened)
{
    PSP_TRACE_SENTINEL();
...
template <typename DATA_T>
void
t_gnode::_process_helper(const t_column* fcolumn,
                         const t_column* scolumn,
                         t_column* dcolumn,
                         t_column* pcolumn,
                         t_column* ccolumn,
